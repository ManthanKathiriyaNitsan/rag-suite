/**
 * RAG SUITE WIDGET LOADER
 * 
 * This is the loader script that users add to their website.
 * It's a small JavaScript file that:
 * 1. Reads configuration from script tag attributes
 * 2. Loads the widget bundle (widget.umd.js)
 * 3. Loads widget CSS (widget.css)
 * 4. Initializes the widget with the configuration
 * 
 * HOW TO USE:
 * 
 * Basic:
 * <script src="https://api.yoursite.com/widget/v1/loader.js" 
 *         data-ragsuite-project-id="your-project-id" 
 *         async></script>
 * 
 * Advanced:
 * <script>
 *   window.ragSuiteConfig = { projectId: "abc123", position: "bottom-right" };
 * </script>
 * <script src="https://api.yoursite.com/widget/v1/loader.js" async></script>
 */
...
(function() {
  ...
  console.log('🚀 RAG Suite: Loader script started');
   ...
   */
  if (window.RAGSuiteLoader) {
    console.warn('RAG Suite: Loader already initialized');
    return;
  }
  
  // Mark loader as initialized
  window.RAGSuiteLoader = true;
  console.log('✅ RAG Suite: Loader marked as initialized');
  ...
  if (!scriptTag) {
    console.error('❌ RAG Suite: Script tag not found. Make sure to include data-ragsuite-project-id attribute.');
    return;
  }
  
  console.log('✅ RAG Suite: Script tag found');
   ...
   * 
   * Users can also set configuration in window.ragSuiteConfig
   ...
   */
  if (!config.projectId) {
    console.error('RAG Suite: projectId is required. Add data-ragsuite-project-id attribute to script tag or set window.ragSuiteConfig.projectId');
    return;
  }
   ...
   * LOAD WIDGET CSS
   * 
   * Load the widget stylesheet first.
   * This ensures styles are available when widget renders.
   */
  const widgetCSS = document.createElement('link');
  widgetCSS.rel = 'stylesheet';
  widgetCSS.href = `${config.apiEndpoint}/widget/${widgetVersion}/widget.css`;
  console.log('📦 RAG Suite: Loading CSS from:', widgetCSS.href);
  widgetCSS.onload = function() {
    console.log('✅ RAG Suite: CSS loaded successfully');
  };
  widgetCSS.onerror = function() {
    console.error('❌ RAG Suite: Failed to load widget CSS from', widgetCSS.href);
    console.warn('RAG Suite: Widget may still work but styling may be incomplete');
  };
   ...
   * This contains React, the widget component, and all dependencies.
   */
  const widgetScript = document.createElement('script');
  widgetScript.src = `${config.apiEndpoint}/widget/${widgetVersion}/widget.umd.js`;
  widgetScript.async = true; // Load asynchronously (don't block page)
  widgetScript.crossOrigin = 'anonymous'; // Allow CORS if needed
  console.log('📦 RAG Suite: Loading widget bundle from:', widgetScript.src);
  
  /**
   * HANDLE WIDGET SCRIPT LOAD SUCCESS
   * 
   * When widget bundle loads, initialize the widget.
   */
  widgetScript.onload = function() {
    console.log('✅ RAG Suite: Widget bundle loaded successfully');
    // Check if widget is available
    if (window.RAGSuiteWidget && window.RAGSuiteWidget.init) {
      console.log('✅ RAG Suite: RAGSuiteWidget found on window object');
      try {
      ...
      console.error('❌ RAG Suite: Widget script loaded but RAGSuiteWidget not found on window object');
      console.error('Available window properties:', Object.keys(window).filter(k => k.includes('RAG')));
      console.error('Check if widget.umd.js loaded correctly and exports RAGSuiteWidget');
    }
  };
  
  /**
   * HANDLE WIDGET SCRIPT LOAD ERROR
   * 
   * If widget fails to load, show error message.
   */
  widgetScript.onerror = function() {
    console.error('❌ RAG Suite: Failed to load widget script from', widgetScript.src);
    console.error('❌ RAG Suite: Check that the widget files are available at the specified endpoint');
    console.error('Possible issues:');
    console.error('  1. Backend not serving files from /api/v1/widget/v1/');
  ...
  console.log('RAG Suite: Loader initialized, loading widget...', {
